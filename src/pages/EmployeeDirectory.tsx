...
export const EmployeeDirectory: React.FC = () => {
  const { employees, addEmployee, setCurrentUser } = useHRMS();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Browse and manage all {employees.length} team members across the organization
          </p>
        </div>
              ...
              <div>
                ...
                <div className="mt-5 space-y-2 text-xs text-slate-600 dark:text-slate-300">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{emp.department} • Reports to {emp.manager}</span>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      {selectedEmployee && (
        <Modal
          ...
          subtitle={`Dayflow Unified HRMS Record • ${selectedEmployee.employeeId}`}
          maxWidth="2xl"
        >
          <div className="space-y-6">
            <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700">
              <img
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="success" size="sm">{selectedEmployee.status}</Badge>
                </div>
                <p className="text-xs text-slate-500">{selectedEmployee.designation} • {selectedEmployee.department}</p>
                <p className="text-[11px] text-slate-400 font-mono">ID: {selectedEmployee.employeeId}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 text-xs">
              <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-400">Email Address</span>
                <div className="font-semibold text-slate-900 dark:text-white mt-0.5">{selectedEmployee.email}</div>
              </div>
              <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-400">Phone Number</span>
                <div className="font-semibold text-slate-900 dark:text-white mt-0.5">{selectedEmployee.phone}</div>
              </div>
              <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-400">Reports To</span>
                <div className="font-semibold text-slate-900 dark:text-white mt-0.5">{selectedEmployee.manager}</div>
              </div>
              <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-400">Work Location</span>
                <div className="font-semibold text-slate-900 dark:text-white mt-0.5">{selectedEmployee.location}</div>
              </div>
              <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-400">Date of Joining</span>
                <div className="font-semibold text-slate-900 dark:text-white mt-0.5">{selectedEmployee.joinDate}</div>
              </div>
              <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800">
                <span className="text-[10px] uppercase font-bold text-slate-400">Annual Compensation</span>
                <div className="font-semibold text-slate-900 dark:text-white mt-0.5">${selectedEmployee.salary.toLocaleString()} USD</div>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-brand-600 bg-brand-50 hover:bg-brand-100 dark:bg-brand-950/40 rounded-xl transition"
              >
                <UserCheck className="w-3.5 h-3.5" />
                Impersonate / Preview As This User
              </button>

              <button
                onClick={() => setSelectedEmployee(null)}
                className="px-4 py-1.5 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 rounded-xl transition"
              >
                Close
              </button>
            </div>
      ...
      <Modal
        ...
        subtitle="Enter workforce details for the new team member"
        maxWidth="2xl"
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/20"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
